...
class CloneSingleRunWithNewDefTester(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CloneSingleRunWithNewDefTester(self):
        sel = self.selenium
	...
	cc().fillDataForTestRun(self,sel,defaulttester=env.useremail2)
	cc().clickActionInCloneTestRun(self,sel,"Save")
	cc().verifyTestRunPageIsReady(self,sel,testrunName,defaulttester=env.useremail2)
