...
class CloneSingleRunWithNewProduct(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CloneSingleRunWithNewProduct(self):
        sel = self.selenium
	...
	cc().fillDataForTestRun(self,sel,product=env.product2)
	cc().clickActionInCloneTestRun(self,sel,"Save")
	cc().verifyTestRunPageIsReady(self,sel,testrunName,product=env.product2)
