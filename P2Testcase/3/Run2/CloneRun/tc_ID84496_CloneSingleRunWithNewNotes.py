...
class CloneSingleRunWithNewNotes(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CloneSingleRunWithNewNotes(self):
        sel = self.selenium
	...
	cc().fillDataForTestRun(self,sel,notes=env.longstring)
	cc().clickActionInCloneTestRun(self,sel,"Save")
	cc().verifyTestRunPageIsReady(self,sel,testrunName,notes=env.longstring)
