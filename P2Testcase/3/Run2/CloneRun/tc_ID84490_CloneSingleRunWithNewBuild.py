...
class CloneSingleRunWithNewBuild(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CloneSingleRunWithNewBuild(self):
        sel = self.selenium
	...
	cc().fillDataForTestRun(self,sel,build=env.build12)
	cc().clickActionInCloneTestRun(self,sel,"Save")
	cc().verifyTestRunPageIsReady(self,sel,testrunName,build=env.build12)
