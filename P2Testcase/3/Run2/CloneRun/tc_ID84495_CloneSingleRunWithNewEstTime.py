...
class CloneSingleRunWithNewEstTime(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CloneSingleRunWithNewEstTime(self):
        sel = self.selenium
	...
	cc().fillDataForTestRun(self,sel,estdays="1",esthours="2",estmins="3",estsecs="10")
	cc().clickActionInCloneTestRun(self,sel,"Save")
	cc().verifyTestRunPageIsReady(self,sel,testrunName,estdays="1",esthours="2",estmins="3",estsecs="10")
