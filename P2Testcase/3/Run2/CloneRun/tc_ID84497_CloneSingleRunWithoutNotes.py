...
class CloneSingleRunWithoutNotes(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CloneSingleRunWithoutNotes(self):
        sel = self.selenium
	...
	cc().createCustomizedTestRunFromPlan(self,sel,testplanName,summary=testrunName,product=env.product1,prodversion=env.prodversion11,build=env.build11,\
		runmanager=env.useremail,defaulttester=env.useremail,notes=env.longstring)
	...
	cc().fillDataForTestRun(self,sel,notes="")
	cc().clickActionInCloneTestRun(self,sel,"Save")
	cc().verifyTestRunPageIsReady(self,sel,testrunName,notes="")
