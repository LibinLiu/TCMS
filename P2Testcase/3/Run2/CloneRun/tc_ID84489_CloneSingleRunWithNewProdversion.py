...
class CloneSingleRunWithNewProdversion(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CloneSingleRunWithNewProdversion(self):
        sel = self.selenium
	...
	cc().fillDataForTestRun(self,sel,prodversion=env.prodversion12)
	cc().clickActionInCloneTestRun(self,sel,"Save")
	cc().verifyTestRunPageIsReady(self,sel,testrunName,prodversion=env.prodversion12)
