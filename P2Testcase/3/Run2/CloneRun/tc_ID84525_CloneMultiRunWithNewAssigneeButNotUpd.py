...
class CloneMultiRunWithNewAssigneeButNotUpd(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CloneMultiRunWithNewAssigneeButNotUpd(self):
        sel = self.selenium
	...
	cc().fillDataForCloneMultiTestRun(self,sel,testrunIdOnoffs={testrunId1:"on",testrunId2:"on"},product=env.product1,prodversion=env.prodversion12,build=env.build12,assignee=env.user2,updateassignee="off")
	...
	#Try to search by 'Case Run Assignee' to make sure the new asssignee info has not been copied into the cloned runs
