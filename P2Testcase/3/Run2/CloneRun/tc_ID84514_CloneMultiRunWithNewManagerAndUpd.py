...
class CloneMultiRunWithNewManagerAndUpd(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CloneMultiRunWithNewManagerAndUpd(self):
        sel = self.selenium
	...
	cc().fillDataForCloneMultiTestRun(self,sel,testrunIdOnoffs={testrunId1:"on",testrunId2:"on"},product=env.product1,prodversion=env.prodversion12,build=env.build12,runmanager=env.user2,updatemanager="on")
	...
	#Try to search by 'Manager' to make sure the new Manager info has been copied into the cloned runs
	cc().fillDataForSearchRun(self,sel,summary=testrunName1,product=env.product1,prodversion=env.prodversion12,build=env.build12,ownertype="Manager",owner=env.user2)
	cc().clickActionInSearchRun(self,sel,"Search")
	cc().verifySearchRunPageIsReady(self,sel,summary=testrunName1,product=env.product1,prodversion=env.prodversion12,build=env.build12,ownertype="Manager",owner=env.user2)
	cc().verifyLink(self,sel,testrunName1)

	cc().fillDataForSearchRun(self,sel,summary=testrunName2,product=env.product1,prodversion=env.prodversion12,build=env.build12,ownertype="Manager",owner=env.user2)
	cc().clickActionInSearchRun(self,sel,"Search")
	cc().verifySearchRunPageIsReady(self,sel,summary=testrunName2,product=env.product1,prodversion=env.prodversion12,build=env.build12,ownertype="Manager",owner=env.user2)
	cc().verifyLink(self,sel,testrunName2)
