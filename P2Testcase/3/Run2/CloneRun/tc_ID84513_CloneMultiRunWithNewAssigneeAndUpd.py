...
class CloneMultiRunWithNewAssigneeAndUpd(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CloneMultiRunWithNewAssigneeAndUpd(self):
        sel = self.selenium
	...
	cc().fillDataForCloneMultiTestRun(self,sel,testrunIdOnoffs={testrunId1:"on",testrunId2:"on"},product=env.product1,prodversion=env.prodversion12,build=env.build12,assignee=env.user2,updateassignee="on")
	...
	#Try to search by 'Case Run Assignee' to make sure the new asssignee info has been copied into the cloned runs
	cc().fillDataForSearchRun(self,sel,summary=testrunName1,product=env.product1,prodversion=env.prodversion12,build=env.build12,caserunassignee=env.user2)
	cc().clickActionInSearchRun(self,sel,"Search")
	cc().verifySearchRunPageIsReady(self,sel,summary=testrunName1,product=env.product1,prodversion=env.prodversion12,build=env.build12,caserunassignee=env.user2)
	cc().verifyLink(self,sel,testrunName1)

	cc().fillDataForSearchRun(self,sel,summary=testrunName2,product=env.product1,prodversion=env.prodversion12,build=env.build12,caserunassignee=env.user2)
	cc().clickActionInSearchRun(self,sel,"Search")
	cc().verifySearchRunPageIsReady(self,sel,summary=testrunName2,product=env.product1,prodversion=env.prodversion12,build=env.build12,caserunassignee=env.user2)
	cc().verifyLink(self,sel,testrunName2)
