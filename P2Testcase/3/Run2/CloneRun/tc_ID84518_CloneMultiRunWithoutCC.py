...
class CloneMultiRunWithoutCC(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CloneMultiRunWithoutCC(self):
        sel = self.selenium
	...
	cc().fillDataForCloneMultiTestRun(self,sel,testrunIdOnoffs={testrunId1:"on",testrunId2:"on"},product=env.product1,prodversion=env.prodversion12,build=env.build12,clonecc="off")
	...
	cc().verifyTestRunPageIsReady(self,sel,testrunName1,product=env.product1,prodversion=env.prodversion12,build=env.build12,nocclist=[env.useremail2])
	...
	cc().verifyTestRunPageIsReady(self,sel,testrunName2,product=env.product1,prodversion=env.prodversion12,build=env.build12,nocclist=[env.useremail,env.useremail2])
