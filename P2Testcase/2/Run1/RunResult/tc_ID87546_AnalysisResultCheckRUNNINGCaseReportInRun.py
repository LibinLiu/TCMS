...
class AnalysisResultCheckRUNNINGCaseReportInRun(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_AnalysisResultCheckRUNNINGCaseReportInRun(self):
        sel = self.selenium
	...
	cc().alterCaseRunStatus(self,sel,"RUNNING")
	...
	cc().verifyReportInfoInRun(self,sel,IDLE="1",RUNNING="1",TOTAL="2")
	
	cc().clickActionInRun(self,sel,"Report")
	cc().verifyTestLogReportPageIsReady(self,sel,testrunId,testrunName,caserunStatusList=["RUNNING","IDLE"])
