...
class AnalysisResultCheckWAIVEDCaseReportInRun(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_AnalysisResultCheckWAIVEDCaseReportInRun(self):
        sel = self.selenium
	...
	#Execute test
	#(1)change status to WAIVED
	cc().selectCaseInRun(self,sel,[testcaserunId1])
	cc().alterCaseRunStatus(self,sel,"WAIVED")
	...
	cc().verifyReportInfoInRun(self,sel,IDLE="1",WAIVED="1",TOTAL="2")
	
	cc().clickActionInRun(self,sel,"Report")
	cc().verifyTestLogReportPageIsReady(self,sel,testrunId,testrunName,caserunStatusList=["WAIVED","IDLE"])
