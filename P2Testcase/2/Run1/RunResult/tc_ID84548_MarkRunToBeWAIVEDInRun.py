...
class MarkRunToBeWAIVEDInRun(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_MarkRunToBeWAIVEDInRun(self):
        sel = self.selenium
	...
	#Execute test
	#(A) - Mark the 1st case in run to be WAIVED
	#expand test case 1
	cc().expandCaseInRun(self,sel,testcaseName1)
	cc().verifyText(self,sel,component1)
	cc().verifyTextNotPresent(self,sel,component2)

	#change test case 1's status to WAIVED
	cc().alterCaseRunStatusByIcon(self,sel,testcaseName1,"WAIVED")

	#Verify if the status has been changed successfully
 	cc().verifyCaseRunStatus(self,sel,testcaseName1,"WAIVED")
	...
	#----------------------------------------
	#(B) - Mark the 2nd case, that is, the last one in run to be WAIVED
	...
	#change test case 2's status to WAIVED
	cc().alterCaseRunStatusByIcon(self,sel,testcaseName2,"WAIVED")

	#Verify if the status has been changed successfully
 	cc().verifyCaseRunStatus(self,sel,testcaseName2,"WAIVED")
