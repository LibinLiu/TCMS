...
class SelCompBySearchForNewCase(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_SelCompBySearchForNewCase(self):
        sel = self.selenium
	...
	cc().addComponentInCreateCase(self,sel,{env.componentId11:env.component11},wayToAdd="Double Click",needSearchComp=True)

	#save the test case
	cc().clickActionInCreateCase(self,sel,"Save")
	cc().verifyTestCasePageIsReady(self,sel,testcaseName,componentIdNames={env.componentId11:env.component11})
