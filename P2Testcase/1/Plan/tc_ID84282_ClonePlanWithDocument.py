...
class ClonePlanWithDocument(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_ClonePlanWithDocument(self):
        sel = self.selenium
	...
	cc().fillDataForCloneTestPlan(self,sel,copyPlanDocument="on")
	cc().clickActionInCloneTestPlan(self,sel,"Clone")
	cc().verifyTestPlanPageIsReady(self,sel,"Copy of "+testplanName)
	cc().verifyDocumentInfoInPlan(self,sel,planDocStr)
