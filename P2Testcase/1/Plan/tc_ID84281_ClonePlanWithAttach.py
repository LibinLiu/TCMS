...
class ClonePlanWithAttach(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_ClonePlanWithAttach(self):
        sel = self.selenium
	...
	cc().fillDataForCloneTestPlan(self,sel,copyPlanAttachments="on")
	cc().clickActionInCloneTestPlan(self,sel,"Clone")
	cc().verifyTestPlanPageIsReady(self,sel,"Copy of "+testplanName,attachmentNameList=[attachmentName],testcaseNameList=[testcaseName1])
