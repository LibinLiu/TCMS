...
class ClonePlanWithEnvironment(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_ClonePlanWithEnvironment(self):
        sel = self.selenium
	...
	cc().fillDataForCloneTestPlan(self,sel,copyEnvironmentGroup="on")
	cc().clickActionInCloneTestPlan(self,sel,"Clone")
	cc().verifyTestPlanPageIsReady(self,sel,"Copy of "+testplanName,envGroup=env.envgroup1)
