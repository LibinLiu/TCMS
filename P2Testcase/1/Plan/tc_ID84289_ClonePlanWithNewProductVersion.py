...
class ClonePlanWithNewProductVersion(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_ClonePlanWithNewProductVersion(self):
        sel = self.selenium
	...
	cc().fillDataForCloneTestPlan(self,sel,prodversion=env.prodversion12)
	cc().clickActionInCloneTestPlan(self,sel,"Clone")
	cc().verifyTestPlanPageIsReady(self,sel,"Copy of "+testplanName,prodversion=env.prodversion12)
