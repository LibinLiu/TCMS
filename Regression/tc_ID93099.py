...
class Bug635549(unittest.TestCase):
    ...
    def test_bug635549(self):
        ...
        for i in range(60):
            try:
                if sel.is_element_present("btn_clone"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        try: self.failUnless(sel.is_element_present("btn_clone"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        sel.click("btn_clone")
        sel.wait_for_page_to_load("30000")
        for i in range(60):
            try:
                if "Clone Plan" == sel.get_title(): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        try: self.assertEqual("Clone Plan", sel.get_title())
        except AssertionError, e: self.verificationErrors.append(str(e))
        for i in range(60):
            try:
                if "Clone Test Plan" == sel.get_text("//div[@id='content']/h2"): break
            except: pass
            time.sleep(1)
        else: self.fail("time out")
        try: self.assertEqual("Clone Test Plan", sel.get_text("//div[@id='content']/h2"))
        except AssertionError, e: self.verificationErrors.append(str(e))
        for i in range(60):
            try:
