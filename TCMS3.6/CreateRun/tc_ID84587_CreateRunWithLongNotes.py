...
class CreateRunWithLongNotes(unittest.TestCase):
    def setUp(self):
        self.verificationErrors = []
        self.selenium = selenium(env.host, env.port, env.browser, env.domain)
        self.selenium.start()
    
    def test_CreateRunWithLongNotes(self):
        sel = self.selenium
	...
	#(2)Execute test
	cc().createCustomizedTestRunFromPlan(self,sel,testplanName,summary=testrunName,notes=env.longstring)
